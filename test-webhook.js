...
async function testWebhook() {
    console.log("Testing SvelteKit webhook...");
    
    try {
        const leadResponse = await fetch("http://localhost:5173/api/zoho-webhook", {
            method: "POST",
        ...
        const noteResponse = await fetch("http://localhost:5173/api/zoho-webhook", {
            method: "POST", 
            headers: { "Content-Type": "application/json" },
        ...
        const chatResponse = await fetch("http://localhost:5173/api/chat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ message: "What do we know about John Doe?", entity: "lead_123" })
        });
